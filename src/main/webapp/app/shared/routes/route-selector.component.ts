...
import { PILOT_ID } from 'app/app.constants';
...
export class RouteSelectorComponent implements OnInit, OnDestroy {
  ...
  setActiveRoute(instance: Route | null): void {
    if (instance && this.routes && this.routes.findIndex(r => r.serviceId === instance.serviceId) !== -1) {
  ...
  private updateRoute(): void {
    ...
    this.routesService
      ...
      .subscribe(
        routes => {
          // adding the pilot self route to the available services routes
          const pilotRoute: Route = PILOT_ID as Route;
          routes.unshift(pilotRoute);
  ...
  getActiveRoute(): string | undefined {
    return this.activeRoute!.serviceId ? this.activeRoute!.serviceId.toUpperCase() : this.activeRoute!.path.toUpperCase();
  }
